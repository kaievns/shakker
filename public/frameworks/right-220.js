...
/**
 * A tiny neat wrapper for methods that take keys-value
 * parameters both as two separated parameters
 * or as a hash of values
 *
 * @param original function
 * @return wrapped function
 */
function dehashify(original) {
  return function(param) {
    if (typeof param === 'string') {
      original.apply(this, arguments);
    } else {
      var key, args = slice.call(arguments, 1);
      for (key in param) {
        original.apply(this, [key].concat([param[key]]).concat(args));
      }
    }
    return this;
  };
...
Element.include({
  ...
  setStyle: function(hash, value) {
    ...
    for (key in hash) {
      c_key = key.indexOf('-') === -1 ? key : key.camelize();
...
Element.include({
   ...
   * @return Element self
   */
  set: dehashify(function(key, value) {
    if (key === 'style') {
      this.setStyle(value);
    } else {
      var element = this._;
